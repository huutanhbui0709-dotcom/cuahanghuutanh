...
const USE_BLOB = IS_VERCEL && VERCEL_BLOB_TOKEN;
...
async function uploadImageFile(file, folder = 'products') {
  ...
  if (vercelBlob) {
    ...
    try {
      await vercelBlob.put(blobPath, file.buffer, { access: 'public' });
      // Trả về URL có thể dùng được
      return `/api/blob-image/${folder}/${filename}`;
    } catch (err) {
