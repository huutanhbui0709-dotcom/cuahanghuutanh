...
async function uploadImageFile(file, folder = 'products') {
  ...
  // Xác minh R2 đã nhận file thành công trước khi trả URL.
  // Nếu không phải 200, throw error để caller KHÔNG ghi URL broken vào DB.
  const httpStatus = result.$metadata?.httpStatusCode;
  if (httpStatus && httpStatus !== 200) {
    throw new Error(`R2 upload thất bại (HTTP ${httpStatus}) cho file: ${rawFilename}`);
  }
