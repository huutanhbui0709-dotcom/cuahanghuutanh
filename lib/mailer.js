...
// Chỉ khởi tạo transporter khi đã có đủ thông tin đăng nhập
...
if (EMAIL_USER && EMAIL_PASS) {
  ...
  console.warn('⚠️ EMAIL_USER hoặc EMAIL_PASS chưa được cấu hình. Tính năng gửi mail đơn hàng sẽ bị bỏ qua.');
}
...
async function sendOrderNotification(order) {
  try {
    if (!transporter) {
      console.warn('⚠️ Gửi mail đơn hàng bị bỏ qua do thiếu EMAIL_USER/EMAIL_PASS.');
      return;
    }

    const info = await transporter.sendMail({
      from: `Cửa Hàng Hữu Tánh <${EMAIL_USER}>`,
      to: NOTIFY_TO, // order hiện chưa có email khách hàng, nên gửi thông báo về email cửa hàng
