// =====================================================================
// MAILER - Gửi thông báo đơn hàng qua Resend API
// =====================================================================
...
async function sendOrderNotification(order) {
  try {
    if (!resend) {
      console.warn('⚠️ Gửi mail đơn hàng bị bỏ qua do thiếu RESEND_API_KEY.');
      return;
    }

    const { data, error } = await resend.emails.send({
      from: 'Cua hang Huu Tanh <onboarding@resend.dev>',
      to: NOTIFY_TO,
    ...
    if (error) {
      console.error(`❌ Lỗi gửi mail qua Resend cho đơn hàng #${order.id}:`, error);
    } else {
      console.log(`📧 Đã gửi mail đơn hàng #${order.id} → ${NOTIFY_TO} (ID: ${data.id})`);
    }
  } catch (err) {
