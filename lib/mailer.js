  ...
  <div style="max-width:640px;margin:24px auto;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.12)">
    ...
    <div style="padding:20px 32px;border-bottom:1px solid #eee">
      ...
      <table style="border-collapse:collapse;width:100%;font-size:14px;font-family:Arial,sans-serif;">
        ...
        <tr><td style="padding:4px 0;color:#888">Địa chỉ:</td><td style="padding:4px 0">${(() => {
          const raw = String(order.address || '');
          const m = raw.match(/\(?\s*(?:To[ạa]\s*độ|T[ọo]a\s*độ)\s*:\s*([-\d.]+)[,\s]+([-\d.]+)\s*\)?/i);
          if (!m) return raw;
          const mainAddr = raw.replace(m[0], '').replace(/[\s,–-]+$/, '').trim();
          const lat = m[1], lon = m[2];
          return (mainAddr ? mainAddr + '<br>' : '') +
            '<span style="color:#555;font-size:13px">Toạ độ: ' + lat + ', ' + lon + '</span><br>' +
            '<a href="https://www.google.com/maps?q=' + lat + ',' + lon +
            '" target="_blank" style="color:#1a73e8;font-size:13px;text-decoration:underline">' +
            '📍 Xem trên Google Maps</a>';
        })()}</td></tr>
        <tr><td style="padding:4px 0;color:#888">Vận chuyển:</td><td style="padding:4px 0">${isFreeShipping ? '<strong style="color:#16a34a;background:#ecfdf5;padding:2px 8px;border-radius:4px;border:1px solid #a7f3d0;display:inline-block">✓ ĐỦ ĐIỀU KIỆN MIỄN SHIP (Đơn ≥ 300k KV Thốt Nốt)</strong>' : 'Giao hàng tiêu chuẩn'}</td></tr>
