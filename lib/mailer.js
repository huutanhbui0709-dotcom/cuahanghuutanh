...
<body style="margin:0;padding:0;font-family:Arial,sans-serif;background:#f0f2f5">
  <div style="max-width:640px;margin:24px auto;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.12)">
    <!-- Header -->
    <div style="background:linear-gradient(135deg,#e65c00,#f9d423);padding:24px 32px">
      <h1 style="margin:0;color:#fff;font-size:22px">🛒 Đơn hàng mới - Cửa hàng Hữu Tảnh</h1>
      <p style="margin:6px 0 0;color:rgba(255,255,255,.9);font-size:14px">Mã đơn: <strong>${order.id}</strong> &nbsp;|&nbsp; ${order.createdAt}</p>
    </div>
...
async function sendOrderNotification(order) {
  try {
    ...
    await transporter.sendMail({
      from: `"Cửa hàng Hữu Tảnh" <${senderEmail}>`,
      to: NOTIFY_TO,
